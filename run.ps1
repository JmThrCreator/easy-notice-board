...
Function find_folder(){
    ...
    $folder_browser = New-Object System.Windows.Forms.FolderBrowserDialog -Property @{
        RootFolder            = "MyComputer"
        Description           = "Select source folder"
        
    }

    if (-Not [String]::IsNullOrWhiteSpace($selected_path))
        {
            $folder_browser.SelectedPath=selected_path
        }

    if($folder_browser.ShowDialog() -eq "OK")
        {
...
Function get_config(){
    ...
    $file = Get-Content .\config.ini
    $ini = Get-Content .\config.ini
    ...
    ForEach($line in $ini_temp){
        $split_array = $line.Split("=")
        $ini_hash += @{$split_array[0] = $split_array[1]}
        If($split_array[0] -eq "SOURCE" -and $split_array[1] -eq {}){
            $ini = Get-IniContent .\config.ini
            $folder = find_folder
            $ini["SOURCE FILE LOCATION"]["SOURCE"] = $folder
